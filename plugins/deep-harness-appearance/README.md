...
| 🎨 **品牌外观** | 设置 → 通用 →「DEEPHARNESS 外观」:固定品牌深蓝顶栏/侧栏色、界面字体一键切换(默认/微软雅黑/宋体/楷体/等宽)、**渐变背景预设**(暗夜蓝/极光紫/深林/纯色深蓝)与**图片背景**(上传 + 固定 16:9 裁剪;仓库自带 `assets/backgrounds/默认.jpg` 为默认背景)、字体导入(仓库 `fonts\` 目录扫描 + 网页上传到 `%USERPROFILE%\.dsh\fonts`) |
...
├─ lib/
│  ├─ index.js         # host 半:webServer 路由 /deepharness/api/*(文件树/读写/终端/字体/背景图)
│  └─ client.js        # 浏览器半:conversation.view 标签 + settings.general.item 设置项 + 16:9 裁剪
└─ README.md
```
...
- **host 半**注册 `/deepharness/api/*` HTTP 路由(与工作台同源):
  `/status`(工作区根)、`/tree`(文件树)、`/file`(读)、`/write`(写)、`/mkdir`、`/exec`(命令执行,PowerShell,工作区根为 cwd)、`/fonts`、`/font`(读)、`/font/upload`(上传)、`/backgrounds`(背景图列表)、`/background`(原始图片字节,供 CSS 引用)、`/background/upload`(上传裁剪产物)。
  背景图目录:仓库 `assets/backgrounds\` + `%USERPROFILE%\.dsh\backgrounds\`;文件操作使用 node:fs 并做工作区包含校验(越界路径一律拒绝);命令执行走 `child_process`。
- **浏览器半**通过 `dsh.client` 机制被页面内核自动发现并加载:
  - `conversation.view` 槽位注册「文件」「终端」标签(与内置"轨迹"标签同协议);
  - `settings.general.item` 槽位注册外观设置(品牌色/字体/渐变/背景图上传与 16:9 裁剪/字体导入);
  - 费用数据来自 DSH 会话投影 `tokenUsage`(真实用量),按官方峰谷定价在浏览器内估算;
  - 背景生效机制:`html/body` 承载背景(渐变或图片),同时用主题令牌覆盖把应用框架/侧栏变为半透明,配合 `backdrop-filter` 毛玻璃。
