...
`;
		...
		// 适配 DEEPHARNESS 运行时:本 DSH 版本的 settings.section 槽位只接受 React 组件
		// (原版用的是旧式 command render() 契约,挂载即触发 React error #130),故把
		// 命令式面板包一层 React 组件(ref 挂载 + unmount 清理);同时原版分区标签与
		// 原生"插件"分区同名导致侧边栏同名两项,改为独立分区「注入器」。
		...
		function mountPanel(root) {
			...
			return () => { try { page.remove(); } catch { /* ignore */ } window.clearInterval(timer); };
		}
		function InjectorSection() {
			const holderRef = useRef(null);
			useEffect(() => {
				if (!holderRef.current) return;
				const dispose = mountPanel(holderRef.current);
				return () => { try { dispose(); } catch { /* ignore */ } };
			}, []);
			return React.createElement("div", { ref: holderRef, style: { maxWidth: 760 } });
		}
		function apply(ctx) {
			ctx.effect(() => ctx.slots.inject("settings.section", () => ctx.slots.register({
				...
				order: 45,
				label: () => "注入器"
			}, InjectorSection)), "super-injector: settings page");
		}
		//#endregion
		exports.apply = apply;
