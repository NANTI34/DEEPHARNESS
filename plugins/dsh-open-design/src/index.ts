...
export function apply(ctx: Context): void {
  ...
  if (!startup || !startup.mode) {
    // DEEPHARNESS adaptation: this profile is not driven by the OpenDesign
    // stdio protocol (no --models/--probe/--stdio startup mode provided), so
    // the runtime stays idle instead of failing the whole profile boot.
