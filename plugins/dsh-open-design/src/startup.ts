...
export function apply(ctx: Context): void {
  ...
  if (!rawArgs.some((arg) => arg === '--models' || arg === '--probe' || arg === '--stdio')) {
    // DEEPHARNESS adaptation: the web profile owns the argument surface
    // (--host/--port/--trusted-host). Parsing unconditionally would reject the
    // web flags here (and the web app's own startup rejects the OD flags),
    // which would break a plain `dsh web` boot. Provide the service with an
    // undefined mode so dependents activate and stay idle.
